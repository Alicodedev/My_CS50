...
void tabulate(void)
{
    for (int i = 0; i < voter_count; i ++){
        for (int j = 0; j < candidate_count; j++){
            if (candidates[preferences[i][j]]).eliminated == false){ // checks if candidates has been eliminated 
                candidates[preferences[i][j]].votes++; // increments vote value for each jth candidate who is not eliminated
                break;
            }
        }
    }
...
int find_min(void)
{
    int min = voter_count; // keep tracking of min value 
